...
exports.update = function(params, json, db, callback) {
    ...
    if (params.hasOwnProperty(json.KEY_ACCOUNT) && params.hasOwnProperty(json.KEY_USERNAME)) {
        var usrName = params[json.KEY_USERNAME];
        if (usrName === '') {
            callback(resp.Resp.errMsg(json, json.VALUE_ERROR_MESSAGE_OF_USER_NAME_EMPTY));
        } else {
...
function processData(params, json, db, callback) {
    ...
    db.tools.dbQuery(conn, sql).then(rows => {
        ...
        if (len > 0) {
            ...
            sql = util.format("UPDATE Info SET UserName = \'%s\', Update_Time = NOW() WHERE Info_ID = %s", params[json.KEY_USERNAME], infoID);
            console.log(sql);
