...
exports.add = function(params, json, accTools, db, callback) {

    if (params.hasOwnProperty(json.KEY_ACCOUNT) && params.hasOwnProperty(json.KEY_DEVICE_NAME) &&
        params.hasOwnProperty(json.KEY_DEVICE_MAC)) {
                ...
                if (len > 0) {
                    ...
                    db.tools.dbQuery(connDev, sql).then(rows => {
                        ...
                        if (len > 0) {
                            ...
                            if (isBind == 1) {
                                sendErrMsg(db, connDev, json, json.VALUE_ERROR_MESSAGE_OF_HAVE_BEEN_BIND, callback);
                            } else {
                                var id = rows[0].ID;
                                sql = util.format("UPDATE Account_Device_Aquarium_Bind SET Device_Name = \'%s\', Is_Bind = 1, Bind_Time = NOW() WHERE ID = %s", params[json.KEY_DEVICE_NAME], id);
                            ...
                            sql = util.format("INSERT INTO Account_Device_Aquarium_Bind (Info_ID, DA_ID, Device_Name, Is_Bind, Bind_Time) VALUES (%s, %s, \'%s\', 1, NOW())", infoID, daID, params[json.KEY_DEVICE_NAME]);
                            db.tools.dbQuery(connDev, sql).then(row => {
                                sendSucMsg(db, connDev, json, callback);
                            }).catch(err => {
                                sendErrMsg(db, connDev, json, json.VALUE_ERROR_MESSGAE_OF_SERVER_EXCEPTION, callback);
