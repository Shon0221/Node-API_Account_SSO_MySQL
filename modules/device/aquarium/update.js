...
exports.update = function(params, json, accTools, db, callback) {

    if (params.hasOwnProperty(json.KEY_ACCOUNT) && params.hasOwnProperty(json.KEY_DEVICE_NAME) &&
        params.hasOwnProperty(json.KEY_DEVICE_MAC) && params.hasOwnProperty(json.KEY_AQUARIUM_TYPE)) {
                ...
                if (len > 0) {
                    ...
                    db.tools.dbQuery(connDev, sql).then(rows => {
                        ...
                        if (len > 0) {
                            ...
                            if (isBind == 1) {
                                var id = rows[0].ID;
                                sql = util.format("UPDATE Account_Device_Aquarium_Bind SET Device_Name = \'%s\', Aquarium_Type = %s WHERE ID = %s", params[json.KEY_DEVICE_NAME], params[json.KEY_AQUARIUM_TYPE], id);
